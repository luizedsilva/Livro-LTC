...
Nessa gramática, $A$ e $B$ são variáveis, enquanto $0$, $1$ e $\#$ são símbolos terminais. A variável $A$ é a variável inicial ou variável de partida.
...
pode ser utilizada para acrescentar simultaneamente um símbolo `0` à esquerda e um símbolo `1` à direita de uma cadeia. A produção $A \rightarrow B$ permite substituir a variável $A$ por $B$, enquanto $B \rightarrow \#$ encerra o processo de geração, produzindo o símbolo terminal `#`.
...
Para a gramática apresentada anteriormente, podemos gerar a cadeia $000\#111$ por meio da sequência:
...
Nesse exemplo, cada aplicação da produção $A \rightarrow 0A1$ acrescenta um `0` no início e um `1` no final. Depois de realizadas as substituições necessárias, a variável $A$ é substituída por $B$, e finalmente $B$ é substituída por `#`.

#### Árvores de derivação

Outra maneira de representar uma derivação é por meio de uma **árvore de derivação**. A raiz da árvore corresponde à variável inicial. Os filhos de um nó correspondem aos símbolos presentes no lado direito da produção utilizada para expandir a variável representada pelo nó.

Para a derivação de $000\#111$, a árvore de derivação representa graficamente a sequência de aplicações das produções. As folhas da árvore, lidas da esquerda para a direita, formam a cadeia gerada pela gramática.
...
podem existir diferentes formas de agrupar as operações. Uma interpretação pode associar primeiro os dois primeiros operandos:
...
Um autômato com pilha pode reconhecer essa linguagem armazenando na pilha um símbolo para cada $0$ encontrado na entrada. Quando começam a ser encontrados os símbolos $1$, o autômato retira um símbolo da pilha para cada $1 lido.
...
A capacidade adicional fornecida pela pilha permite reconhecer outras linguagens que não podem ser reconhecidas por autômatos finitos. Um exemplo apresentado nos slides é a linguagem
...
Inicialmente, o símbolo inicial da gramática é colocado na pilha. O autômato utiliza as produções da gramática para substituir uma variável pelo lado direito de uma produção. Esse processo pode ser realizado por transições que não consomem símbolos da entrada.
...
### Forma simplificada de um Autômato com Pilha

Para facilitar a construção de uma GLC equivalente, é conveniente considerar uma forma simplificada de autômato com pilha. Os slides apresentam três condições para essa forma.

A primeira condição é que o autômato possua **um único estado de aceitação**. Isso simplifica a identificação das computações completas que correspondem a cadeias aceitas.

A segunda condição é que a pilha esteja **vazia antes da aceitação**. Assim, uma computação aceita pode ser caracterizada tanto pelo estado final quanto pelo esvaziamento da pilha.

A terceira condição estabelece que cada transição deve realizar uma única operação sobre a pilha: **empilhar ou desempilhar**, mas não realizar as duas operações simultaneamente.

Essas condições não alteram a classe de linguagens que pode ser reconhecida. Elas apenas tornam a estrutura do autômato mais conveniente para a transformação em uma gramática livre de contexto.

### Exemplo de transformação de AP para GLC

Considere um autômato com estados numerados, para o qual são introduzidas variáveis associadas a pares de estados. Se os estados forem $0$, $1$ e $2$, por exemplo, poderão ser consideradas as variáveis

$$
A_{00},A_{11},A_{22},A_{01},A_{02},A_{12}.
$$

Se o autômato começa no estado $0$ e deve terminar no estado $2$, a variável inicial será

$$
A_{02}.
$$

A partir das transições do autômato, são construídas produções como
...
Também são introduzidas produções que representam computações vazias, como
...
O conjunto dessas produções descreve, por meio de derivações da gramática, as possíveis computações do autômato. Dessa forma, uma cadeia pertence à linguagem do autômato exatamente quando pode ser gerada pela gramática construída.
