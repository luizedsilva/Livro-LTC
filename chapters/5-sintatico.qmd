...
A partir dos conceitos apresentados, podemos usar a ferramenta **bison** para construir como um exemplo uma calculadora de expressões aritméticas. Esse processo é ilustrado na @fig-flex-bison.

![Processo de construção da calculadora](/chapters/images/flex-bison.png){#fig-flex-bison width="90%"}
...
Assim, o arquivo escrito pelo programador (`calc.l`) constitui a especificação do analisador, enquanto `lexico.c` corresponde ao código-fonte C produzido automaticamente pelo Flex.
...
O ponto indica a posição corrente durante a construção do item LR. Nesse caso, o analisador encontra-se no estado inicial e ainda não reconheceu nenhum símbolo da produção.
...
A partir dessa declaração, o Yacc identifica `NUMERO` como um dos terminais utilizados pela gramática. Durante a geração do analisador, são produzidas as estruturas necessárias para que o analisador léxico e o analisador sintático utilizem uma representação comum dos tokens.
...
* `$2` representa o valor associado ao token `MAIS`;
...
o valor do token `NUMERO` é simplesmente transferido para o não terminal `expr`. Essa ação permite que o valor numérico reconhecido pelo analisador léxico seja propagado pela árvore sintática durante as reduções.
...
Os símbolos terminais (alfabeto da gramática) estão representados pelas constantes com o prefixo `T_`como: T_PROGRAMA, T_INICIO, T_FIM, T_IDENTIF, T_INTEIRO, T_LOGICO, T_LEIA, T_ESCREVA, T_ENQTO, T_FACA, T_FIMENQTO, T_SE, T_ENTAO, T_SENAO, T_FIMSE, T_ATRIB, T_VEZES, T_DIV, T_MAIS, T_MENOS, T_MAIOR, T_MENOR, T_IGUAL, T_E, T_OU, T_V, T_F, T_NAO, T_ABRE e T_FECHA. Essas constantes representam codificações numéricas dos símbolos que são reconhecidos e retornados pelo analisador léxico.
